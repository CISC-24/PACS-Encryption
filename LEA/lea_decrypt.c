...
int main(void)
{
	...
	if ((dir = opendir(encrypted_path)) != NULL) {
		while ((ent = readdir(dir)) != NULL) {
			const char *file_ext = strrchr(ent->d_name, '.');

			if (file_ext != NULL && strcmp(file_ext, ".enc") == 0) {
				char enc_img_path[1024];
				...
				fread(enc_img_data, 1, img_size, enc_img_file);
				fclose(enc_img_file);
